...
write.table(data_tot,paste("../../../Teich_resultsLFS/simulated_timeseries_very_skewed_SAD/MockData_SAD_2sp_antiphase_log.csv",sep=""),sep=";",dec=".",row.names=F)

tab=cbind(log_mu,SigmaPair)
colnames(tab)=c("log_mu",paste("Sp",1:nspecies))
write.table(tab,paste("../../../Teich_resultsLFS/simulated_timeseries_very_skewed_SAD/MuSigma_SAD_2sp_antiphase_log.csv",sep=""),sep=";",dec=".",row.names=F,append=T)
} #end r

pdf("MockData_2sp_antiphase_timeseries_log.pdf",width=20,height=6)
