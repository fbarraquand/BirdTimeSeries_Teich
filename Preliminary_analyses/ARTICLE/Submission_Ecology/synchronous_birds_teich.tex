...
\author{{\Large{}Frédéric Barraquand$^{1,2,*}$, Coralie Picoche$^{1}$,
...
very frequently observed outcome on biodiversity time series is synchrony
\citep{vasseur_synchronous_2014}. Synchrony occurs
...
and this explanatory factor that has been somewhat neglected for now. On
...
sensu} \citealt{gonzalez2009causes}) so that individuals from species A or B would experience similar competition. 
Priority effects and chance events could then determine whether duck species A or B dominates.
...
This should occur because more dissimilar species are more likely to have different environmental preferences and
...
of changes in vegetation height). Surprisingly, such compensation \textit{between}
...
dissimilar guilds \citep[e.g.][]{sinclair2013asynchronous}. In this paper, we
...
levels), favoring different types of wetland birds (so that over
...
comprises 120 ha of wetlands, and the counts have been aggregated at
...
\citet{loreau_species_2008}'s $\phi$),
...
\citep{hallett_codyn_2016}. This is particularly important here
...
coot \emph{(Fulica atra}, an abundant species here). We also ``zoomed in'' on a group of species that were known to exhibit
...
was rejected at the Bonferroni-corrected 10\% threshold. In addition to the time-domain analyses above, we performed frequency-domain
...
\url{https://github.com/fbarraquand/BirdTimeSeries_Teich}\footnote{Made public upon acceptance and archived in Zenodo}.
...
from the null hypothesis (no temporal correlation between species), i.e. there
...
community (Fig A1 in Appendix 1). For the cold season, \textit{Calidris} and \textit{Anas} exhibit opposite
...
coarser scales, over several years. When we consider the wavelet plot (Fig. \ref{fig:Wavelet-modulus-ratio}),
that is a time-varying and scale-dependent strength
of synchrony, we can see that there is synchrony even at a fine temporal
...
only two functional groups, the Gross index reduces to a simple correlation. Waders and ducks are negatively correlated during the cold season
...
% regions significantly different from the null model (independently fluctuating species) with a false discovery
...
% group formed by cormorant, egret and heron (see the captions of Fig. \ref{fig:Gross-synchrony-index} and Fig. \ref{fig:Wavelet-modulus-ratio} for symbol interpretation) \label{fig:Time-domain-(top)-and}}
% \end{figure}
...
6 years) there seems to be some compensation, which may indicate a progressive
...
annual scale or below. Yet, summing species abundances within a guild and comparing the ``biomass
...
local dynamics. Zooming in on the cormorant-heron-egret module, we find that compensation
...
season (C. Feigné, pers. obs.), which mostly occurred before 2006. 
...
\hspace{-1cm}
%\begin{centering}
\includegraphics[width=20cm]{Fig1.pdf}
%\par\end{centering}
\caption{}
% \caption{Time series of counts for\emph{ }ducks of the genus\emph{ Anas} (a),
% calidrids (b, \emph{Calidris} genus), and all waders (c, including
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=1\textwidth]{Fig2_new2}
\par\end{centering}
\caption{}
% \caption{Gross' synchrony index ($\eta$) as a function of the season (cold
% and warm seasons), calculated within (top, a-b) and between (bottom,
...
\par\end{centering}
\begin{centering}
\includegraphics[width=0.8\textwidth]{wavelet_triad}
\par\end{centering}
\vspace{1cm}
\caption{}
% \caption{Time-domain (a) and frequency-domain (b) synchrony analyses of the
