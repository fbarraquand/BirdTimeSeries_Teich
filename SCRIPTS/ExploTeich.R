...
DBori=DB # store original file in memory
...
hist(DB$Annee, main = "Distribution of records") ### All species at once
unique(DB$Nom_latin) ### how many & which species are there

# A few 'easy' species to explore
...
unique(DB$ID.Lieu.dit) # "Lieu dit" is the spatial sampling unit
unique(DB$Lieu_dit) # Many of them
...
# histogram(log(n_obs)) # (less pretty)
...
write.csv2(table_sampling_lieux_dits_Teich,file = "../OUT/Table_sampling_lieux_dits_withinTeich.csv")
...
write.csv(DB,file = "../IN/DBWithMonthlyPhotoTeich_protocol2corrected.csv")
...
pdf("../OUT/photo/TrendsAbundance_Teich_monthly_photograph.pdf",width=8,height=12)
...
## File with data that were missing, communicated later by Claude Feigné. 
...
## To check the database, I aligned the old database and the new.
## No difference: before 2006 and after 2008.
## Between 2006 and 2008, differences for some species, a lot of new points (good !)
...
DB1<-read.csv(file="../IN/DBWithMonthlyPhotoTeich_protocol2corrected.csv",header=TRUE,sep=",",dec=".")
