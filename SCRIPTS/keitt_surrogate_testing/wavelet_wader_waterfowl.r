## CP 08/07/2020 Script to plot the two wavelet analyses of waders and waterfowl together, later adapted to a smaller grid
...
type_correct="BH" #was Bonferroni before, applied only on the Gross test. For wavelet, there is no correction.
anrands=1000
end_bio="abundances"
normalize_seq=c(T,F)
doyouload=F #If TRUE, load previous results. If FALSE, relaunch all analyses
...
pdf(paste("Submission_JAE/Revisions_R2/wavelet_wader_waterfowl",end_nor,"with",anrands,"rands_nocorrection_smallergrid.pdf",sep=""),height=15,width=12)
