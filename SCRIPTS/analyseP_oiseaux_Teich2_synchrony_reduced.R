####CP 06/09/2019: This is a cleaned up version of analyseP_oiseaux_Teich2_synchrony.R (entirely written by CAluome) Sto keep only what we need and use in the article (the rest might be put in exploratory analyses) 
...
xmin = date("1981-01-01")
xmax = date("2015-01-01")
plot(df_HeronEgret$Date,log10(df_HeronEgret$Nombre),col="darkorchid",xlab="Year",ylab="Log10(abundance)",xlim=c(xmin,xmax),cex.lab=1.8,cex.axis=1.5,xaxt="n",pch=19,ylim=c(ymin,ymax),type="o",cex=0.9,las=1,lwd=2)
lines(df_HeronEgret$Date,log10(df_HeronEgret$Nombre+1),col="darkorchid",lwd =1,lty=3)
lines(abondance_phalacrocorax$Date,log10(abondance_phalacrocorax$Nombre),col="blue",type='o',lwd=2,pch=19,cex=0.9)
